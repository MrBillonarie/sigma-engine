#!/usr/bin/env python3
"""
fetch_dukascopy_xag.py
Descarga datos historicos XAGUSD desde Dukascopy (gratis, desde 2014).
Genera: data_XAG_5m, 15m, 1h, 4h _max.csv con 10+ años de historia.
...
LOG_PATH    = Path('/opt/sigma/results/reports/dukascopy_xag_fetch.log')
PROGRESS_F  = Path('/tmp/dukascopy_xag_progress.json')
CACHE_DIR   = Path('/tmp/duka_xag_cache')
...
def duka_url(year, month0, day, hour):
    return f"https://datafeed.dukascopy.com/datafeed/XAGUSD/{year}/{month0:02d}/{day:02d}/{hour:02d}h_ticks.bi5"
...
def merge_and_save(tf, duka_bars, label=""):
    ...
    csv_path = OUTPUT_DIR / f'data_XAG_{tf}_max.csv'
    if csv_path.exists():
...
def save_all_tfs(all_bars_dict, label=""):
    ...
    try:
        df_1h = pd.read_csv(str(OUTPUT_DIR / 'data_XAG_1h_max.csv'), index_col=0, parse_dates=True)
        ...
        df_4h.to_csv(str(OUTPUT_DIR / 'data_XAG_4h_max.csv'))
...
def main():
    log.info("=" * 60)
    log.info("DUKASCOPY XAG FETCHER — XAG Silver 5m/15m/1h/4h desde 2014")
