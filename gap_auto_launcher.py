...
# --- SIGMA VITRINA gap hook ---
def _vitrina_log_gap(sym, tf, direction, strategy=None):
    try:
        from utils.decisions import log_decision
        log_decision(kind='gap_closed', slot=f'{sym}|{tf}',
                     payload={'sym': sym, 'tf': tf, 'direction': direction, 'new_strategy': strategy},
                     meta={'source': 'gap_auto_launcher'})
    except Exception:
        pass
...
def _gap_priority(sym, tf, direction, regime):
    ...
    tf_score = {'5m': 80, '15m': 120, '1h': 150, '4h': 150}.get(tf, 5)
...
for sym, tf, direction in todo[:slots_to_launch]:
    ...
    full_sym = SYM_MAP.get(sym, sym + '/USDT')
    csv_arg = ''
    if sym in CSV_PATHS:
        csv_arg = '--csv_path ' + CSV_PATHS[sym].replace('{tf}', tf) + ' '
    cmd = (
    ...
    if rc.returncode == 0:
        ...
        try:
            from utils.decisions import log_decision
            log_decision(kind='push_started', slot=f'{sym}|{tf}',
                payload={'sym': sym, 'tf': tf, 'direction': direction, 'trials': TRIALS_PER_SLOT, 'attack_type': 'gap'},
                meta={'source': 'gap_auto_launcher'})  # --- VITRINA gap_attack call ---
