...
# 04:30 diario: git commit local + push vps-production a GitHub (offsite)
...
# ── Push a GitHub rama vps-production (offsite) ───────────────────────────────
git push origin vps-production >> $LOG 2>&1 && \
    echo "[$DATE] GitHub push OK (vps-production)" >> $LOG || \
    echo "[$DATE] GitHub push FALLO (se reintentara manana)" >> $LOG

# ── Code tar.gz local (recovery rapido sin clonar git) ───────────────────────
mkdir -p $BACKUP_DIR
tar czf "$CODE_TAR" \
